...
var config = {
    ...
    // 默认菜单配置
    menus: ['head', 'bold', 'italic', 'underline', 'strikeThrough', 'foreColor', 'backColor', 'link', 'list', 'justify', 'quote', 'emoticon', 'image', 'table', 'video', 'code', 'undo', 'redo', 'toggle'],
...
function Toggle(editor) {
    this.editor = editor;
    this.$elem = $('<div class="w-e-menu" style="white-space: nowrap;">切换模式</div>');
    this.type = 'click';
...
Toggle.prototype = {
    ...
    onClick: function onClick() {
        var editor = this.editor;
        if (editor.customConfig.toggle) {
          editor.customConfig.toggle();
        }
    },
}
...
MenuConstructors.toggle = Toggle;
...
UploadImg.prototype = {
    ...
    uploadImg: function uploadImg(files) {
        ...
        var uploadImgFieldName = config.uploadImgFieldName || 'picture';
        var uploadDataType = config.uploadDataType || 'json';
        ...
        arrForEach(resultFiles, function (file) {
            var name = uploadImgFieldName || file.name;
            ...
            if (uploadImgServer && typeof uploadImgServer === 'string') {
                ...
                xhr.onreadystatechange = function () {
                    ...
                    if (xhr.readyState === 4) {
                        ...
                        if (!hooks.customInsert && result.status != 0) {
                            // hook - fail
                            if (hooks.fail && typeof hooks.fail === 'function') {
                                hooks.fail(xhr, editor, result);
                            }
                ...
                var requestData = {};
                requestData[uploadImgFieldName] = base64.split(',')[1];
                objForEach(uploadImgParams, function (key, val) {
                    requestData[key] = val;
                });

                // 发送请求
                xhr.send(uploadDataType === 'json' ? JSON.stringify(requestData) : formdata);
